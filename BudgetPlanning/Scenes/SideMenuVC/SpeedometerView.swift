...
final class SpeedometerView: UIView {
    ...
    private let minAngle = 180 * CGFloat.pi / 180
    private let maxAngle = 360 * CGFloat.pi / 180
...
//        layer.mask = trackLayer
        layer.masksToBounds = true
//        let endAngle = isClockWise ?  CGFloat((1.5-delay) * .pi) : CGFloat((-0.5+delay) * .pi)
        let circlePath = UIBezierPath(arcCenter: CGPoint(x: frame.size.width/2, y: frame.size.height/2), radius: (frame.size.width)/2, startAngle: 0.5 , endAngle: 2.0 , clockwise: true)
...
//        trackLayer.addSublayer(marksLayer)
//        trackLayer.addSublayer(minorMarksLayer)
//        speedLabels.values.forEach { addSubview($0) }
//        trackLayer.addSublayer(arrowLayer)
//        trackLayer.addSublayer(centerPointLayer)
